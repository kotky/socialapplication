    ...
    if request.user.is_authenticated():
        ...
        for post in all_posts_and_comments:
            ...
            if post.parent is None:
                post_dict["post_"+str(post.id)]={"id":post.id,"creator_id":post.user.id, "creator_username":post.user.username, "text":post.text,"image":post_image,"likes_count":len(post_likes), "liked_alredy":liked_alredy, "comments":[], "date_created_ms": datetime_to_ms_str(post.date_created),"date_created":post.date_created}
            else:
                post_dict["post_"+str(post.parent.id)]["comments"].append({"id":post.id,"creator_id":post.user.id, "creator_username":post.user.username, "creator_image":SocialUser.objects.get(user=post.user).image.url, "text":post.text,"image":post_image,"likes_count":len(post_likes), "liked_alredy":liked_alredy, "date_created_ms": datetime_to_ms_str(post.date_created), "date_created":post.date_created})
...
def publish_comment(request):
    ...
    print recieptant_list
    for user in recieptant_list:
        result = client.publish("User_"+str(user.id), {"event":"comment_added","data":{"type": "post", "creator_id":request.user.id, "creator_username":request.user.username, "text":post.text, "id":post.id, "date_created":datetime_to_ms_str(post.date_created), "image_url": post_image_url}})
    result = client.publish("User_"+str(request.user.id),  {"event":"comment_added","data":{"type": "post", "creator_id":request.user.id, "creator_username":request.user.username, "text":post.text, "id":post.id, "date_created":datetime_to_ms_str(post.date_created), "image_url": post_image_url}})
